...
const host = 'http://127.0.0.1:5500/architecture&Testing/exercises/softTerest/index.html'; // Application host (NOT service host - that can be anything)
const DEBUG = false;

import data from './mock-data.json' assert { type: 'json' };
let _ideas = data.ideas;
let _details = data.details;
// import { ideas as _ideas, details as _details } from './mock-data.json' assert { type: 'json' };
const endpoints = {
...
describe('E2E tests', function () {
    ...
    beforeEach(async () => {
        ...
        await context.route('**' + endpoints.ideas, route => route.fulfill(json(_ideas)));
        await context.route('**' + endpoints.details + '*', route => route.fulfill(json(_details)));
    ...
    describe('Catalog', () => {
        ...
        it('show idea details', async () => {
            ...
            expect(title).to.equal(_details.title);
            expect(desc).to.equal(_details.description);
            expect(img).to.equal(_details.img);
        });
    ...
    describe('CRUD', () => {
        ...
        it('non-author does NOT see delete button', async () => {
            const mock = Object.assign({}, _details, { _ownerId: '0002' }); // Replace mock with non-owned object
