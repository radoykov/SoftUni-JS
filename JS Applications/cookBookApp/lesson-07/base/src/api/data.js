...
export async function getRecipes(page = 1, search) {
    if (!search) { return await proFetch(urls.recipesWithPager(page), 'get'); }

    return await proFetch(urls.recipesWithPager(page) + '&where=' + encodeURIComponent(`name like "${search}"`), 'get');
}
...
export async function getRecipeCount(search) {
    if (!search) { return await proFetch(urls.recipesCount, 'get'); }

    return await proFetch(urls.recipesCount + '&where=' + encodeURIComponent(`name like "${search}"`), 'get');
}
